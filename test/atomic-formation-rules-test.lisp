...
(defun test-restrict-type-basic (&optional verbose)
  ...
  (let* ((animal-type (get-concept-type 'animal))
         ...
         (restricted-concept (restrict-concept-type animal-concept dog-type)))
    ...
    (and (not (eq animal-concept restricted-concept))  ; Different objects
         (types-equal (concept-type restricted-concept) dog-type)  ; Restricted type
         (referents-equal (referent animal-concept) (referent restricted-concept)))))  ; Same referent
...
(defun test-restrict-referent-generic-to-individual (&optional verbose)
  ...
  (let* ((dog-type (get-concept-type 'dog))
         ...
         (restricted-concept (restrict-concept-referent generic-concept spot-indiv)))
    ...
    (and (not (eq generic-concept restricted-concept))
         ...
         (not (null (referent restricted-concept)))
         (individuals-equal (content (referent restricted-concept)) spot-indiv))))
...
(defun test-restrict-referent-compatible-individuals (&optional verbose)
  ...
  (let* ((dog-type (get-concept-type 'dog))
         ...
         (restricted-concept (restrict-concept-referent basic-concept detailed-dog)))

    (when verbose
      (format t "~&Basic dog concept: ~A~%" basic-concept)
      (format t "~&Restricted to detailed: ~A~%" restricted-concept))

    (and (not (eq basic-concept restricted-concept))
         (types-equal (concept-type basic-concept) (concept-type restricted-concept))
         (individuals-equal (content (referent restricted-concept)) detailed-dog))))
