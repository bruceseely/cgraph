...
(defun init-test-graphs ()
  ...
  ;;concepts
  (setf chevy-con  (make-concept 'chevy ()))
...
(defun test-graph-4 ()
  ...
  (let ((graph-string "[PERSON: Sue]<-(agnt)<-[GIVE]-
    ...
    (set-arc-from-relation food-con inst-rel)
    (set-arc-from-relation food-con obj-rel)
    (add-arc-into-relation eat-con obj-rel)

    ;; [EAT:*x]->(manr)->[FAST]
    (add-arc-into-relation eat-con manr-rel)
    (set-arc-from-relation fast-con manr-rel)
...
(defun test-graph-12 ()
  ...
  (let ((graph-string "[DRIVE]-
    ...
    (set-arc-from-relation chevy-con inst-rel)


    ;; [PERSON]->(pos)->[CHEVY]
    (add-arc-into-relation person-con poss-rel)
    (set-arc-from-relation chevy-con  poss-rel)
