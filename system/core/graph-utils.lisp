      ...
      (loop
        ...
        (flet ((match (types)
          ...
          (cond
            ...
            (t (unless *in-graph-referent*
                 (format t "~2% ---- unhandled by linkup ------~%")
...
(defmethod path ((tokens list))
  (when (every (lambda (token)
                 (or (typep token 'arc)
                     (not (null (arcs token)))))
               tokens)
    (remove-if (lambda (x) (typep x 'arc)) tokens)))
...
(defun linkup-strings (&rest strings)
  (funcall #'linkup (mapcar (lambda (string)
                              (nodes (pcg string)))
                              strings)))

(defmethod format-path ((nodes list))
  (remove #\space (apply #'strcat (format-path-tokens (path nodes)))))
...
(defmethod find-concept (concept-type (graph graph) &key id (properties (list)) (ignore-num-refs (list)))
  (find (get-concept-type concept-type) (graph-concepts graph) :key #'concept-type :test #'eql))

(defmethod find-concept (concept-type (concept-list list) &key id (properties (list)) (ignore-num-refs (list)))
  (find (get-concept-type concept-type) concept-list :key #'concept-type :test #'eql))
...
(defmethod walk-graph+ ((node graph-node) function)
  (let* ((seen (list))
         (agenda (list node))
         current-node)
    (block nil
...
(defmethod collect-concepts ((graph graph-node))
  (remove-if-not (lambda (node)
                   (typep node 'concept))
                 (collect-nodes graph)))

(defmethod collect-concepts ((graph graph))
  (collect-concepts (head graph)))
...
(defmethod identical-objects-p ((object1 concept) (object2 concept))
  ...
  (referents-equal (referent object1) (referent object2)))
...
;; (defmethod graphs-equal ((g1 graph) (g2 graph) &optional debug)
;;   (graphs-equal (car (nodes g1)) (car (nodes g2)) debug))
...
(defmethod compare-graphs ((g1 graph-node) (g2 graph-node))
  (let ((result (cond ((graphs-eq (graph-head g1) (graph-head g2)))
                      ((graphs-equal (graph-head g1) (graph-head g2)) :equal))))
