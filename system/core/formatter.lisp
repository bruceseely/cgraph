...
(defun arc-string (node second)
  (typecase node
    (concept
     (cond ((nodes-equal node (outarc second))
            left-arrow-string)
	   (t
            right-arrow-string)))
    (relation
     (cond ((nodes-equal second (outarc node))
            right-arrow-string)
	   (t
            left-arrow-string)))))
...
(defun format-path-tokens (path)
  (let* ((text-list (list))
    ...
    ;; Note: removed single-node handling - let the main loop handle all remaining nodes

    (do ((node-list (nthcdr nodes-processed path) (cdr node-list)))
        ((null node-list))
      ;;(format t "~&DEBUG: main loop processing node-list = ~s~%" node-list)
      ...
      (let ((first-node (car node-list)))
        ...
        ;; (format t "~&text-list: ~s~%"  text-list)
        )
...
(defmethod format-cgraph ((node graph-node) &key (initial-indent 0) (indent-delta 3))
  (let* ((segments (make-graph-segments node))
         ...
         (graph-text (strcat (string-trim (list #\, #\- #\space #\newline) base-text) ".")))
