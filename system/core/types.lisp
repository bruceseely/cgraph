...
(defmethod make-top-concept-type ()
  (let ((concept-type (make-instance 'concept-type :label 'T)))
    (record-concept-type concept-type)
    (setf *concept-type-top* concept-type)
    concept-type))

(defmethod make-bottom-concept-type ()
  (let ((concept-type (make-instance 'concept-type :label '%)))
    (record-concept-type concept-type)
...
(defmethod make-concept-type-aux ((type-label string)  &key supertypes-list canonical-graph-string)
...
(defmethod make-concept-type-aux (type-label &key supertypes-list (canonical-graph-string ""))
  (let ((concept-type (make-instance 'concept-type
				     :label type-label
                                     :supertypes (unless (eq type-label 't)
                                                   (list *concept-type-top*))
                                     :subtypes (unless (eq type-label '%)
...
(defmethod make-concept-type ((concept-type concept-type) &rest keys &key supertypes-list canonical-graph-string)
  (declare (ignore keys supertypes-list canonical-graph-string ))
  concept-type)

(defmethod make-concept-type ((type-label string) &rest keys &key supertypes-list canonical-graph-string)
  supertypes-list canonical-graph-string keys
  (make-concept-type-aux type-label))


(defmethod make-concept-type ((type-label symbol) &rest keys &key supertypes-list canonical-graph-string)
  (when (eq type-label 't)
    (print "------------------------------------------------ make-concept-type~%")
    ;;(print-backtrace)
    )
  supertypes-list canonical-graph-string
...
(defmethod modify-concept-type ((node concept-type)
  ...
  (when supertypes
    (remove *concept-type-top* (direct-supertypes node))
...
(defun define-concept-type (&key label supertypes cgraph)
  ...
  ;; (multiple-value-bind (node error-msg)
...
(defun parse-concept-type-def (def)
  (destructuring-bind (&key label supertypes cgraph) def
    ;;(format t "~&~a ~a ~a" label supertypes cgraph) ; debug
    (let ((type-label (intern (string-upcase label))))
      (define-concept-type :label type-label :supertypes supertypes :cgraph cgraph))))
...
(defun walk-concept-types (function &optional (node (get-concept-type 't)))
  (let ((*visited-nodes* (list (make-bottom-concept-type))))
    (walk-concept-types-aux function node)))

(defun walk-concept-types-aux (function &optional (node (get-concept-type 't)))
...
(defun walk-concept-types-down (function &optional (node (get-concept-type 't)))
  (unless (bottom-concept-type-p node)
    (funcall function node)
    (dolist (subnode (direct-subtypes node))
      (walk-concept-types-down function subnode))))

(defun walk-concept-types-up (function &optional (node (get-concept-type '%)))
...
(defmethod crawl-concept-types-up (function &optional (start-node (get-concept-type '%)))
  (let ((seen-list (list))
...
(defmethod crawl-concept-types-down (function &optional (start-node (get-concept-type 't)))
  (let ((seen-list (list))
        (horizon (list start-node)))
    (block loop
      (loop
...
(defmethod crawl-concept-types (function)
  (crawl-concept-types-down function (get-concept-type 't)))
...
(defmethod type-path ((concept-type concept-type))
  (let ((*type-paths* (list)))
    (type-path-aux concept-type (get-concept-type 't) (list))
    *type-paths*))
  ...
  (unless (eql node *concept-type-bottom*)
    (with-output-to-string (stream)
      ...
      (when (canonical-graph-string node)
        (format stream " :cgraph \"~a\"" (canonical-graph-string node)))
      (format stream ")"))))
...
(defmethod show-relation-types ((source-type symbol) (dest-type symbol) &optional verbose)
  (when (and source-type dest-type)
    (let* ((rels
             (remove-if-not (lambda (rel-type)
                              (and
                               (every (lambda (source-concept-type)
                                        (subtype-p  source-concept-type  source-type ))
                                      (source-types rel-type))
                               (subtype-p (label (dest-type rel-type)) dest-type )))
                            (mapcar #'get-relation-type (all-relation-types))))
           (sorted (sort rels  #'alpha-lessp :key (lambda (x) (princ-to-string (label x))))))
      (when verbose
        (dolist (rel sorted)
          (let ((node (get-relation-type rel)))
            (format t "~&~a:~6t ~a~%" (label node) (desc node)))))
      sorted)))
...
(defun lookup-concept-type (concept-type &optional supress-warnings)
  (handler-case (get-concept-type concept-type)
    (concept-type-lookup-failed ()
      (cond ((string-equal concept-type "nil") nil) ;for monads
            (t
...
(defun parse-relation-def (def &optional supress-warnings)
