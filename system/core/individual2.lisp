...
(defclass individual (referent-mixin)
  ((id :initarg :id
       :type number
       :accessor id)
   (type :initarg :type
...
;;       (format stream "INDIV ~a" (label (concept-type object)))
;;       (dolist (prop props)
...
(defmethod individuals-equal ((ind1 individual) (ind2 individual) &key (ignore-id t))
  (and
   (types-equal (concept-type ind1) (concept-type ind2))
...
(defmethod get-individuals (type (properties list))
  (let* ((ctype (concept-type type))
         ...
         (individuals (remove-if-not
                       (lambda (indiv)
                         (and
                          (types-equal (concept-type indiv) ctype)
                          (equalp (properties indiv) props)))
                       *individuals*)))
    individuals))

;;; The info being parsed may not contain an id
(defmethod get-individual (concept-type &key id properties)
...
(defmethod make-referent ((prop-list list) &optional concept)
  (let* ((props (sans-prop prop-list :id :variable))
...
(defmethod format-individual ((individual individual))
  (let* ((separator (if *concise* "" " "))
         ...
         (id-string (cond ((numberp id) (format nil "~a#~d" separator id))
                          ((eql id 't) (format nil "~a#" separator))
                          ((null id) "")))
...
#|
(defun tst (type &optional properties id)
  (let* ((indiv (make-individual (get-concept-type type)
                                 (list* :id id properties)))
         (text (format-individual indiv)))
    (values indiv text)))


(tst 'dog)
(tst 'dog '(:name "fido"))
(tst 'dog '(:name "fido" :id 456))
(tst 'dog '(:id 345))
|#
...
;;; in graph-nodes.lisp
(defmethod parse-properties ((referent-string string) &key variable)
  variable
  (with-input-from-string (stream referent-string)
...
;;; return a string representation of referent-properties
...
;;; parse the display referent-string representation
;;; create a referent-spec from the properties extracted from the supplied string
(defmethod parse-properties ((referent-string string) &key variable)
  (let ((components (isolate-properties referent-string))
        (properties (list))
        variable)
...
(defmethod conforms ((concept-type1 concept-type) (concept-type2 concept-type))
  (subtype-p concept-type1 concept-type2))
...
(defmethod conforms ((individual individual) (properties list))
  (cond ((null (properties individual)) t)
        (t
         ...
         (properties-equal (properties individual) properties :ignore-id t))))
